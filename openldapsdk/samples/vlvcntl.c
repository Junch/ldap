...
int main( int argc, char **argv)
{
     ...
     */
    rc = ldap_parse_sortresponse_control(
            ld,                     /* Session handle */                   
    ...
    if (LDAP_SUCCESS != rc)
    {
        printf("\n\tldap_parse_sortresponse_control: %s\n", ldap_err2string(rc));
        goto cleanup;
    }
    if (err)
     ...
     */
    rc = ldap_parse_vlvresponse_control(
            ld,                     /* Session handle */
            *resultControls,         /* Results from the search operation */
    ...
    if (LDAP_SUCCESS != rc)
    {
        printf("\n\tldap_parse_vlvresponse_control: %s\n", ldap_err2string(rc));
        goto cleanup;
    }
    if (err)
